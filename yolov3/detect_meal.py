...
def ToF(file, cat):
    ...
    file_name_without_ext = os.path.splitext(file_name)[0]  # 확장자를 제외한 파일 이름 추출
        ...
        for i, det in enumerate(pred):  # detections for image i
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    object_conf.append(label.split()[-1])
                    count = count + 1
                    ...
                    for i in range(count):  ##리스트 두 개 xml파일에 저장
                        ...
                        SubElement(object_xml, 'conf').text = object_conf[i]
                        ...
                        if object_names[i] == "00000000":
                            continue

                        x = int(str(total[i][0]))
                        y = int(str(total[i][1]))
                        w = int(str(total[i][2])) - int(str(total[i][0]))
                    ...
                    if save_txt:  # Write to file
                        with open(save_path[:save_path.rfind('.')] + '.json', 'a') as file:
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        # print('command : python3 detect_meal.py --cfg {0} --names {1} --weights {2}'.format(opt.cfg, opt.names, opt.weights))
        # print('===============================================================')
        #print(d.isoformat())
        # def printOsInfo():
